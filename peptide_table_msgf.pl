...
my $usage='perl /gscmnt/gc2108/info/medseq/proteogenomics/analyses/pipeline_step_1_peptides_detection/cptac/breast $type_mass, $out_file
'; 

die $usage unless @ARGV == 3;
my ( $run_dir, $type_mass, $out_dir) = @ARGV;
...
if($type_mass==0) { $run_dir_msgf=$run_dir."/phosphoproteome"; $out_file=$out_dir."/Phosphoproteome_msgf.txt";  }
else {
     if($type_mass==1) { $run_dir_msgf=$run_dir."/proteome"; $out_file=$out_dir."/Proteome_msgf.txt"; }
     else { die $usage; }
     }
